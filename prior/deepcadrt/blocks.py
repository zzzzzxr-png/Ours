"""3D U-Net building blocks (trimmed from pytorch-3dunet buildingblocks)."""
...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding=1):
    """
    Create a list of modules that together constitute a single conv layer with
...
class SingleConv(nn.Sequential):
    """
    Basic convolutional module: Conv3d + non-linearity + optional batchnorm/groupnorm.
    The order of operations is specified via the `order` parameter.
    """
...
class DoubleConv(nn.Sequential):
    """
    Two consecutive convolution layers (default Conv3d+ReLU).
    Uses padded convolutions so (H_out, W_out) matches (H_in, W_in).
    """
...
class Encoder(nn.Module):
    """
    A single encoder-path module: an optional pooling layer followed by a
    DoubleConv module.
    """
...
class Decoder(nn.Module):
    """
    A single decoder-path module: an upsample step (nearest-neighbor interpolation
    with concatenation joining for DoubleConv) followed by a DoubleConv module.
    """
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
        ...
        if basic_module == DoubleConv:
            ...
            # otherwise use ConvTranspose3d and summation joining
