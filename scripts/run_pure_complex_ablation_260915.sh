...
root=experiments/260915_dtcwt_complex_pure_l1l2_width32_lr5e-5_robust_clip
python=/home/zxr/.conda/envs/physics/bin/python

run_one() {
    local gpu="$1" freq="$2" mask="$3" tag="$4" normalize="$5"
    ...
    CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 "$python" -u \
        ...
        --n_epochs 50 --lr 5e-5 --b1 0.5 --b2 0.999 \
        ...
        --adaptive-grad-clip --grad-clip-warmup-iters 1000 \
        --grad-clip-percentile 95 --grad-clip-warmup-cap 2e6 \
        --grad-clip-median-multiplier 2 \
        --seed 1024 --no_resume \
        "${normalize_arg[@]}" \
        2>&1 | tee "$run_dir/train.log" &
    echo "launched ${freq}Hz mask=${mask} normalize=${normalize} GPU=${gpu} pid=$!"
}

run_one 0 0.1 spatial_mask 0p1Hz false
run_one 1 1 temporal_mask 1Hz false
run_one 2 10 temporal_mask 10Hz false
run_one 3 0.1 spatial_mask 0p1Hz_channel_norm true
run_one 4 1 temporal_mask 1Hz_channel_norm true
run_one 5 10 temporal_mask 10Hz_channel_norm true
