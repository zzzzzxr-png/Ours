...
root=experiments/260920_steerable_fourier_complex_srdtrans_polyphase_highpass_norm_ckpt_patch128_pure_l1l2_lr5e-5

if [[ -e "$root" && "${ALLOW_EXISTING:-0}" != 1 ]]; then
    echo "refusing to overwrite $root (set ALLOW_EXISTING=1 only to resume this directory)" >&2
    exit 1
fi
